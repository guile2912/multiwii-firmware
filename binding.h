...
void myEEPROMwrite(int16_t addr, uint8_t data)
{
  ...
  //if(eeprom_read_word((uint16_t*)addr) != sensorValue) {
    eeprom_write_block((const void*)&data, (void*)addr, sizeof(data));
  //}
  sei();
  /*
  uint8_t retries = 5;
  while ((--retries) && (data !=  eeprom_read_block((const void*)&data, (void*)addr, sizeof(data)))) {
...
{
  uint32_t temp = 0;
  temp = eeprom_read_dword((uint32_t*) EEPROM_OFFSET);
...
/*
  ...
  temp = eeprom_read_dword((uint32_t*)EEPROM_RX_OFFSET);
